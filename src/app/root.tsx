...
if (
  import.meta.env.DEV &&
  import.meta.env.NEXT_PUBLIC_ENABLE_DESIGN_MODE === "true"
) {
...
export const meta = ({ location }: MetaArgs) =>
  buildMeta({ url: buildCanonicalUrl(location?.pathname) });
...
function InternalErrorBoundary({ error: errorArg }: Route.ErrorBoundaryProps) {
  ...
  const copyButtonPaddingClass = shouldScale
    ? "px-[10px] py-[5px]"
    : "px-[6px] py-[3px]";
    ...
    <html lang="en">
      <head>
        ...
        {import.meta.env.DEV ? (
          <script
            type="module"
            src="/src/__create/dev-error-overlay.js"></script>
        ) : null}
